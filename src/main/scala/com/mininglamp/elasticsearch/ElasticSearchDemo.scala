...
object ElasticSearchDemo {
  ...
  def process(input: String): String = {
    ...
    msg_profile.append("src", fields {
      0
    })
    msg_profile.append("tel", fields {
      1
    })
    msg_profile.append("submit_time", fields {
      2
    })
    msg_profile.append("smscontent", fields {
      3
    })
    msg_profile.append("id", fields {
      4
    })
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      */
      ...
      val msgProfileInput = sc.textFile("hdfs:///user/hive/warehouse/data.db/mt_info_day")
      ...
      /*
      val msgProfileInput = "hdfs:///user/hive/warehouse/data.db/mt_info_day"
      val conf = new SparkConf().setAppName("Customer Profile Load").set("es.nodes", "localhost").set("es.port", "9200")
      ...
      //EsSpark.saveToEs(msgProfile, "testmsgprofile/)
      */
